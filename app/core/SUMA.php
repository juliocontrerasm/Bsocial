<?php
    ...
    if ($ajaxAction != ""){
		...
		if ($ajaxAction == 'update'){
			...
			//echo "UPDATE $table SET $field = \"$val\" WHERE $pk = $sql_id"; exit;
		...
		if ($ajaxAction == 'delete'){
			...
			//qr("UPDATE $table SET deleted = '1'  WHERE $pk = $sql_id");
			echo $table . "|" . $id;
		}
...
class ajaxCRUD{
    ...
    function ajaxCRUD($item, $db_table, $db_table_pk, $ajaxcrud_root = "") {
        ...
        $this->emptyTableMessage = "No hay datos en esta tabla. Haga clic en Agregar.";
		...
		if ($this->field_count == 0){
			$error_msg[] = "No hay campos en esta tabla!";
			echo_msg_box();
			exit();
		}
    ...
    function turnOffAjaxADD($id = 'id', $value = 0){
        $this->ajax_add = false;
        $this->get_id = $id;
    ...
    function insertHeader($ajax_file = "ajaxCRUD.inc.php"){
       ...
       /* echo "<link href=\"" . $this->ajaxcrud_root . "css/default.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\" />\n";
        echo "<link href=\"" . $this->ajaxcrud_root . "css/bootstrap-responsive.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\" />\n";
        echo "<link href=\"" . $this->ajaxcrud_root . "css/bootstrap.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\" />\n";*/
	...
	function doAction($action){
		...
		if ($action == 'delete' && $_REQUEST['id'] != ''){
			...
			if ($success){
				...
				$error_msg[] = "$item no se pudo eliminar. Porfavor intente de nuevo.";
			}
		}//action = delete
            ...
            if ($_REQUEST['table'] == $this->db_table){
                ...
                if ($string_submitted_values != ''){
                    ...
                    if ($success){
                        ...
                        $error_msg[] = "$item no pudo ser agregado. Porfavor, intente de nuevo.";
                    }
                }
                else{
        ...
        if ($action == 'delete_file' && $_REQUEST['field_name'] && $_REQUEST['id'] != ''){
            ...
            if ($success){
                ...
                $report_msg[] = "Archivo Elimado Exitosamente";
                ...
                $error_msg[] = "Se ha producido un error al eliminar el archivo.";
            }
	...
	function showTable($deleted = true, $mostrar = true){
        ...
        */
        ...
        if (count($this->ajaxFilter_fields) > 0){
            ...
            $top_html .= "<table  class='busqueda' cellspacing='5' align='right'><tr>";
		...
		if($this->sql_where_clause == ''){
			if($deleted == true)
			{
				$this->sql_where_clause = " WHERE deleted = 0";
			}
		}
		else{
			if($deleted == true)
			{
				$this->sql_where_clause .= " AND deleted = 0";
			}
		}
        $sql = "SELECT * FROM " . $this->db_table . $this->sql_where_clause  . $this->sql_order_by;
        //echo $sql;exit;
        ...
        if ($row_count > 0){
            ...
            */
            ...
            $table_html .= "<table align='center' class='table table ajaxCRUD' name='table_" . $this->db_table . "' id='table_" . $this->db_table . "' cellspacing='" . $this->cellspacing . "' border=" . $this->border . ">\n";
            ...
            foreach ($rows as $row){
                ...
                if ($this->delete || (count($this->row_button)) > 0){
                    ...
                    if ($this->delete){
                        $table_html .= "<input type=\"button\" class=\"btn-sm btn-warning \" onClick=\"confirmDelete('$id', '" . $this->db_table . "', '" . $this->db_table_pk ."');\" value=\"  x  \" />\n";
                    }
                    ...
                    if (count($this->row_button) > 0){
                        foreach ($this->row_button as $the_row_button){
                            ...
                            $table_html .= "<input type=\"button\" $javascript_for_button class=\"btn editingSize btn-success \" value=\"$value\" />\n";
                        }
                    }
		...
		if ($this->showCSVExport) {
			...
			$add_html .= "  <input type=\"submit\" name=\"submit\" value=\"Exportar a CSV\" class=\"btn editingSize btn-info\"/>\n";
			$add_html .= "</form>\n";
			$add_html .= "</center>\n";
		}
        ...
        if ($this->add){
            $add_html .= "<center>\n";
            $add_html .= "   <input type=\"button\" value=\" Agregar $item\" class=\"btn editingSize btn-info\" onClick=\"$('#add_form_$this->db_table').slideDown('slow');\">\n";
            ...
            $add_html .= "<form action=\"" . $_SERVER['PHP_SELF'] ."?".$this->get_id."=".$this->get_value."#ajaxCRUD\" id=\"add_form_$this->db_table\" method=\"POST\" ENCTYPE=\"multipart/form-data\" style=\"display:none;\">\n";
            $add_html .= "<br /><h4>Ingresar $item</h4>\n";
            ...
            foreach($this->add_fields as $field){
            	if($this->get_id == $field){
	            	$add_html .= '<input type="hidden" name="'.$field.'" value="'.$this->get_value.'" />';
            	}
            	else{
                ...
                } //si viene el campo en get setea ese por defecto
            }//foreach
			...
			$add_html .= "<input class=\"editingSize\" type=\"button\" onClick=\"validateAddForm('$this->db_table', $postForm);\" value=\"Guardar $item\">";
        ...
        */
        ...
        if ($this->add_form_top){
        		...
        		$html =  $top_html . $add_html. $table_html . $bottom_html;
	...
	function getQuery($sql,$deleted = true)
	{
		//$sql = "SELECT * FROM " . $this->db_table . $this->sql_where_clause;
		//echo $sql;
		$Rows = q($sql);
		return $Rows;
    }
    
	function getArray($deleted = true)
	{
		$sql = "SELECT * FROM " . $this->db_table . $this->sql_where_clause;
		//echo $sql;
		$Rows = q($sql);
    ...
    function getGraph($deleted = true,$graph_name,$graph_type,$title, $name_title, $value_title, $name, $value)
	{
		$Rows = $this->getArray($deleted);
    ...
    <script type=\"text/javascript\">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['$name_title','$value_title'],";
        for($i = 0; $i < count($Rows); $i++)
        {  
         	$html .= " ['".$Rows[$i]["$name"]."', ".$Rows[$i]["$value"]."],";
        }
       $html .= "]);
        ...
        var chart = new google.visualization.$graph_type(document.getElementById('$graph_name'));
        chart.draw(data, options);
      }
    </script>
    <div id=\"$graph_name\" style=\"width: 900px; height: 500px;\"></div>";
		echo $html;
    }
...
function hace($fecha_unix){
          ...
          function fecha_pro($fecha){
            ...
            if($diferencia_segundos<60){
                ...
                return $diferencia_minutos." minutos.";
                }else if($diferencia_horas<24){
                    return $diferencia_horas." horas.";
                }else {
