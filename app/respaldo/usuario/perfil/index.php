<?php
	...
	if($_POST['nombre']!=null):
		...
		if ($_POST['nuevo_password']!=null):
			...
			if($usuarios!=null):
				$usuario->getQuery("UPDATE `usuarios` SET `password`=".md5($_POST['nuevo_password'])." , `nombre`='".$_POST['nombre']."',`apellido`='".$_POST['apellido']."',`fecha_nacimiento`='".$_POST['año'].'-'.$_POST['mes'].'-'.$_POST['dia'].",`direccion`=".$_POST['direccion']."',`id_comuna`=".$_POST['comuna'].",`cargo`='".$_POST['cargo']."' WHERE id=".$_SESSION['id_usuario']);
				header('Location:../perfil/?save=1');
			else:
				header('Location:../perfil/?error=1');
			endif;
		else:
			$usuario->getQuery("UPDATE `usuarios` SET `nombre`='".$_POST['nombre']."',`apellido`='".$_POST['apellido']."',`fecha_nacimiento`='".$_POST['año'].'-'.$_POST['mes'].'-'.$_POST['dia']."',`direccion`='".$_POST['direccion']."',`id_comuna`=".$_POST['comuna'].",`cargo`='".$_POST['cargo']."' WHERE id=".$_SESSION['id_usuario']);
			header('Location:../perfil/?save=1');
		endif;
	endif;
?>
Menu:  <a href="../estados" title="Ver estados">Estados</a> | <a href="../mensajes/" title= "ve a mensajes">Mensajes</a> | <a href="../login/logout.php" title= "Cerrar sesion">Cerrar sesion </a>
	...
	<?php if($_GET['error'] == 1 ){ ?>
		<h3>Su contraseña no coincide</h3>
	<?php } ?>  
	<?php if($_GET['save'] == 1 ){ ?>
		<h3>Sus datos fueron Guardados con exito</h3>
	<?php } ?>


	<form action="" method="post" id="editar" onsubmit="return validar();">
		...
		<div id ="region-content" class="region">
	   	...
	   	<div id ="comuna-content" class="comuna">
			...
			<br />
			<label id ="comuna-label" class="comuna" for="comuna" >Region</label>
			<br />
		...
		<div id ="r-nuevo-password-content" class="r-nuevo-password" >
			...
			<br />
			<label id ="r-nuevo-password-label" class="r-nuevo-password" for="r-nuevo-password" >Reepita nuevo password</label>
			<br />
